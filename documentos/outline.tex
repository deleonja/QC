...
\begin{align}
  \rho = \frac{1}{2^n}\1 + \sum _{i=1}^{2^{2n}-1}\tau _i\sigma _i,
	\label{rho}
\end{align}
where we will identify $\sigma _i$ as the tensor products of Pauli 
matrices, which form a basis of the space of traceless Hermitian
%\cpnote{Quitar density}
%density 
matrices, and 
$\tau_i$ are the projections of $\rho$ onto each element of this basis.
...
We'll refer to the coefficients in both terms of 
...
In between them, from left to right, patterns represent 
bit-phase flip, phase-flip and bit-flip channels, 
all of them for $p=0.5$.	
These last three channels are equivalent via permutation
...
\begin{enumerate}
	...
	interpreted as transpositions (exploiting the matrix properties of
	patterns).
...
decir que 63 de los 128 son canales.} \janote{Listo.}
...
% \newline
\end{center}
which is in fact another PCE channel.
% \janote{Fin.}
...
PCE channels are the only non-trivial candidates to
...
\end{align}
where $\lambda_J$ are the eigenvalues of a given map, and $W_J$ are 
operators acting on $\mathbb{C}^d$, vector space that has a collection of 
at most $d+1$ mutually unbiased bases $\qty{\ket{\psi_k^J}}$, that are defined as
\begin{align}
	W_J &= \sum _{k=1}^{d}\omega^k \dyad{\psi_k^J}{\psi_k^J}, 
	\hspace{2cm} J=1,\ldots,d+1
\end{align}
where $\omega = e^{2\pi i/d}$. When $d+1$ mutually unbiased bases exist,
the $d^2-1$ unitaries
$\qty{W_J^m}_{m=1,\ldots,d,J=1,\ldots,d+1}$ 
and the identity matrix form a basis for the space of 
Hermitian matrices of $d\times d$. 
%\fxnote{Por fas
...
the number of all $\lambda_J$ different from zero. Note that 
there are $d-1$ eigenvalues that are $\lambda_J$ for each $J$. Consequently, rank 
of maps in $\R$ is $1 + k(d-1)$, where $k = 1,2,\ldots,d+1$. It follows
that a necessary condition for maps in $\R$ and PCE channels to have the 
same rank is that
\begin{align}
